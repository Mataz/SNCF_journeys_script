# Based on http://www.xavierdupre.fr/app/ensae_teaching_cs/helpsphinx2/notebooks/TD2A_eco_
# API_SNCF_corrige.html
# Making my own script to fetch certain data between Compiègne and Paris Nord
...
def page_gares(numero_page):
    return requests.get('https://api.sncf.com/v1/coverage/sncf/stop_areas?start_page={}'
                        .format(numero_page), auth=(token_auth, ''))
...
for page in range(int(total_items/item_per_page)+1):
            ...
            for var_api, var_df in zip(['insee', 'name', 'label', 'id', 'zip_code'],
                ...
                try:
                    ...
                    if var_api not in print_done:
                        print("key '{0}' not here but {1}".format(var_api, ','.join
                        (station['administrative_regions'][0].keys())))
    ...
    try:
        ...
        # SNCF modifie parfois le schéma de ses données
...
print(df.columns)
print(df.shape)
print(df.iloc[651])     # Print Compiègne Pandas purely integer-location
